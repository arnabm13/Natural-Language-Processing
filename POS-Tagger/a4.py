...
def make_feature_dicts(data,w2v_model,
    ...
    for elements in data:
        ...
        for index, list_tuple in enumerate(elements):
            ...
            if  w2v==True:
                if list_tuple[0] in w2v_model:
                    for number in range(0,50):
                        line_dict['w2v_'+str(number)]= w2v_model.wv[list_tuple[0]][number]
            if context == True:
                prev = index - 1
                next = index + 1
                if index > 0:
                    ...
                    if w2v==True:
                        if elements[prev][0] in w2v_model:
                            for number in range(0, 50):
                                line_dict['prev_w2v_' + str(number)] = w2v_model.wv[elements[prev][0]][number]
                ...
                if index < s_len:
                    ...
                    if w2v==True:
                        if elements[next][0] in w2v_model:
                            for number in range(0, 50):
                                line_dict['next_w2v_' + str(number)] = w2v_model.wv[elements[next][0]][number]
...
def evaluate_combinations(train_data, test_data,w2v_model):
    ...
    features_values = [[True,False],[True,False],[True,False],[True,False],[True,False]]
    ...
    for i,element in enumerate(list_features_values):
        ca_value, p_value, ch_value, co_value, w2v_value = element
        combinations_average_f1s,n_params=evaluate_combinations_average_f1s(train_data,test_data,w2v_model,token=True,caps=ca_value,pos=p_value,chunk=ch_value,context=co_value,w2v=w2v_value)
        df.loc[i]=[combinations_average_f1s,n_params,ca_value,p_value,ch_value,co_value,w2v_value]
...
if __name__ == '__main__':
    ...
    w2v_model= gensim.models.Word2Vec(sentences, min_count=5,size=50,window=5)
    # model.save('D:/Assignment_NLP/amukhopa01/a4/brown_model')
    train_data = read_data('train.txt')
    dicts, labels = make_feature_dicts(train_data,w2v_model,
                                       token=True,
                                       caps=True,
